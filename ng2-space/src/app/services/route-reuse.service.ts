 ...
 * @Last Modified time: 2020-07-14 14:49:18
 * @Description: 路由复用策略
 * // TODO: 未完成路由通配符, 以及id路径参数只缓存一个页面
 */
...
/** 路由复用工厂函数 */
export const RouteReuseServiceFactory = (handlerSize = 10) => (
  class RouteReuseService implements RouteReuseStrategy {
    ...
    private static runNgOnDestroy(route: Route) {
      if (!route) {
        return
      }
      // 删除handlers中的缓存
